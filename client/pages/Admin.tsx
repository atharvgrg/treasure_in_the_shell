...
import { Terminal, Trophy, Clock, Users, RefreshCw, Crown, Medal, Award, Trash2 } from 'lucide-react';
        ...
        <div className="container mx-auto px-3 sm:px-4 py-3 sm:py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1 sm:gap-2">
              ...
              <div>
                <h1 className="text-sm sm:text-lg lg:text-xl font-mono font-bold text-primary">ADMIN DASHBOARD</h1>
                <p className="text-xs sm:text-sm text-muted-foreground font-mono hidden sm:block">Treasure in the Shell - Team Progress</p>
                <p className="text-xs text-muted-foreground font-mono sm:hidden">Team Progress</p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6 mb-6 sm:mb-8">
          <Card className="bg-card/50 border-primary/20">
            <CardContent className="p-3 sm:p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs sm:text-sm font-mono text-muted-foreground">Total Teams</p>
                  <p className="text-lg sm:text-2xl font-bold text-primary">{stats.totalTeams}</p>
                </div>
          ...
          <Card className="bg-card/50 border-terminal-400/20">
            <CardContent className="p-3 sm:p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs sm:text-sm font-mono text-muted-foreground">Highest Level</p>
                  <p className="text-lg sm:text-2xl font-bold text-terminal-400">{stats.highestLevel}</p>
                </div>
          ...
          <Card className="bg-card/50 border-orange-400/20">
            <CardContent className="p-3 sm:p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs sm:text-sm font-mono text-muted-foreground">Average Level</p>
                  <p className="text-lg sm:text-2xl font-bold text-orange-400">{stats.averageLevel}</p>
                </div>
          ...
          <Card className="bg-card/50 border-blue-400/20">
            <CardContent className="p-3 sm:p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs sm:text-sm font-mono text-muted-foreground">Total Progress</p>
                  <p className="text-lg sm:text-2xl font-bold text-blue-400">{stats.completedLevels}</p>
                </div>
                ...
                {progressData.map((team, index) => (
                  ...
                  >
                    <div className="flex items-center gap-2 sm:gap-4 min-w-0 flex-1">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-xs font-mono text-muted-foreground">
                          ...
                          <span className="hidden sm:inline">{team.timestamp.toLocaleString()}</span>
                          <span className="sm:hidden">{team.timestamp.toLocaleDateString()} {team.timestamp.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-1 sm:gap-3 flex-shrink-0">
                      ...
                      >
                        <span className="hidden sm:inline">Level {team.level}</span>
